...
* **Modern Frontend:** A snappy, responsive UI built with htmx and Tailwind CSS, providing a "single-page-app" feel without complex JavaScript.
...
* **Translation Service:** **Lingo.dev SDK**
    * Used for all language detection and translation.
...
* An API key from [Lingo.dev](https://lingo.dev/)
...
2.  **Proxy (Node.js):** Create a file at `lingo-proxy/.env` and add your Lingo.dev key:
    ```
...
**A) The Easy Way (VS Code "Launch BOTH Servers")**
1.  Open the project in VS Code.
2.  Go to the **Run and Debug** tab (the play button with a bug).
3.  Select **"Launch BOTH Servers (Project PolySearch)"** from the dropdown.
4.  Press the **Green Play Button (F5)**.

**B) The Manual Way (Two Terminals)**
